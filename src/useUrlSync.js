 ...
 *
 * @param state      {teamKey, season, tab, includeOlder}
...
export function useUrlSync(state, onRestore, storedTeamKey) {
  const { teamKey, season, tab, includeOlder } = state
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      const next = toSearch({ teamKey, season, tab, includeOlder }, window.location.search)
  ...
  }, [teamKey, season, tab, includeOlder])
